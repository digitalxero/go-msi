...
func p10Base(t *testing.T) *msiPackage {
	...
	c.WithFile("a.exe", FileSourceFromBytes([]byte("MZ original a content")))
	c.WithFile("b.dat", FileSourceFromBytes([]byte("shared b content")))
...
func p10Upgraded(t *testing.T) *msiPackage {
	...
	b := NewPackage().
		...
		WithVersion("1.0.1")
	c := b.RootDirectory("INSTALLFOLDER", "App").Component("Main").AssociateToFeature("F")
	c.WithFile("a.exe", FileSourceFromBytes([]byte("MZ NEW a content, longer than before")))
	c.WithFile("b.dat", FileSourceFromBytes([]byte("shared b content")))
	c.WithFile("c.dat", FileSourceFromBytes([]byte("brand new c content")))
	b.Feature("F").WithLevel(1)
	pkg, err := b.Build()
	require.NoError(t, err)
	return pkg.(*msiPackage)
...
func TestPatch_RejectsComponentRemoval(t *testing.T) {
	...
	d.Component("Main").AssociateToFeature("F").WithFile("a.exe", FileSourceFromBytes([]byte("a")))
	d.Component("Extra").AssociateToFeature("F").WithFile("x.dat", FileSourceFromBytes([]byte("x")))
